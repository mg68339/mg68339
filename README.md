<h1 align="center">Hi 👋, I'm Mohit Gupta</h1>
<h3 align="center">Aspiring Data Analyst | Turning Data into Insights 📊</h3>

---

### 👨‍💻 About Me
- 🎓 B.E. in Information Technology (Mumbai University)
- 📊 Passionate about Data Analysis, Visualization & Business Insights
- 🔍 Currently learning **SQL, Python, Power BI, Tableau**
- 🚀 Transitioning from Frontend Development → Data Analytics
- 📈 Love working with data to solve real-world problems

---

### 🔭 Projects
- 📊 **Power BI Dashboard Project**  
  Built interactive dashboards from raw Excel data (data cleaning, transformation, visualization)

- 🗂️ **SQL Data Analysis Projects**  
  Worked on real-world datasets using joins, window functions, aggregations

- 🌐 [Event Management Website](https://github.com/mg68339/Event-Management-System) *(Previous Project)*

---

### 🌱 Currently Learning
- Advanced SQL (Window Functions, Optimization)
- Python for Data Analysis (Pandas, NumPy)
- Data Visualization & Storytelling
- Business Case Studies

---

### 🛠️ Tools & Technologies
<p align="left">
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" width="40" height="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" width="40" height="40"/>
<img src="https://img.icons8.com/color/48/power-bi.png" width="40" height="40"/>
<img src="https://img.icons8.com/color/48/tableau-software.png" width="40" height="40"/>
<img src="https://img.icons8.com/color/48/microsoft-excel-2019.png" width="40" height="40"/>
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" width="40" height="40"/>
</p>

---

### 📫 Connect with Me
<p align="left">
<a href="https://www.linkedin.com/"><img src="https://img.icons8.com/color/48/linkedin.png" width="40"/></a>
<a href="mailto:mg68339@gmail.com"><img src="https://img.icons8.com/color/48/gmail-new.png" width="40"/></a>
<a href="https://instagram.com/moohhhiiittt"><img src="https://img.icons8.com/color/48/instagram-new.png" width="40"/></a>
</p>

---

### 📊 What I Bring
- ✔ Data Cleaning & Transformation  
- ✔ Dashboard Creation (Power BI / Tableau)  
- ✔ SQL Querying & Analysis  
- ✔ Basic Python for Data Analysis  
- ✔ Strong Problem-Solving Skills  

---

### ⚡ Fun Fact
I enjoy turning messy data into meaningful insights 🚀
